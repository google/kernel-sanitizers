									...
									const unsigned long *entries)
{
	...
	trace_hash = jhash2((u32 *)trace_begin, n_entries * sizeof(unsigned long) / sizeof(u32),
							STACK_CACHE_HASH_SEED);
						  ...
						  struct stack_cache_response *entries, unsigned int nentries)
{
	...
	for_each_possible_cpu(cpu) {
		...
		while (true) {
			...
			next_idx = (next_idx + PER_CPU_RING_BUFFER_SIZE - hdr->length)
				& (PER_CPU_RING_BUFFER_SIZE - 1);
			if (hdr->record_type != BUF_RECORD_INFO) {
				/* TODO: add processing of BUF_RECORD_STACK records. */
				continue;
			}
