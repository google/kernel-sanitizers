...
static void flush_echoes(struct tty_struct *tty)
{
	...
	if ((!L_ECHO(tty) && !L_ECHONL(tty)) ||
	    READ_ONCE(ldata->echo_commit) == ldata->echo_head)
		return;

	mutex_lock(&ldata->output_lock);
	WRITE_ONCE(ldata->echo_commit, ldata->echo_head);
...
{
	struct n_tty_data *ldata = tty->disc_data;
	int amt = data_race(poll && !TIME_CHAR(tty) && MIN_CHAR(tty) ? MIN_CHAR(tty) : 1);
