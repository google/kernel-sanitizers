...
static inline void set_other_info(const volatile void *ptr, size_t size,
...
static inline bool use_other_info(const volatile void *ptr, size_t size,
				  bool value_change)
{
	if (!matching_access((unsigned long)other_info.ptr, other_info.size,
	...
	if (IS_ENABLED(CONFIG_KCSAN_REPORT_VALUE_CHANGE_ONLY) &&
	    other_info.is_write && !value_change) {
		/*
		 * The other access is a write, but the data value we observed
		 * did not change. Discard this other_info.
		 */
		return false;
	}
...
static bool set_or_acquire_other_info(unsigned long *flags,
				      const volatile void *ptr, size_t size,
				      bool is_write, bool value_change,
				      int cpu_id, enum kcsan_report_type type)
{
	...
	for (;;) {
		...
		case KCSAN_REPORT_CONSUMED_WATCHPOINT:
			...
			set_other_info(ptr, size, is_write, cpu_id);
			spin_unlock_irqrestore(&other_info_lock, *flags);
			/* other_info may now be consumed */
			return false;
		...
		case KCSAN_REPORT_RACE_SIGNAL:
			...
			/*
			 * First check if this is the other_info we are
			 * expecting, i.e. matches based on how watchpoint was
			 * encoded.
			 */
			...
			if (!use_other_info(ptr, size, value_change)) {
			...
			/*
			 * Matching & usable access in other_info: keep
			 * other_info_lock locked, as this thread consumes it to
			 * print the full report; unlocked in end_report.
			 */
			return true;
		  ...
		  bool value_change, int cpu_id, enum kcsan_report_type type)
{
	...
	if (set_or_acquire_other_info(&flags, ptr, size, is_write, value_change,
				      cpu_id, type)) {
