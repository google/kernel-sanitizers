...
#define KT_ATOMIC_OP(op, ad, size, mo, read, write)                            \
	do {                                                                   \
		kt_tab_sync_t *sync = NULL;                                    \
                                                                               \
		sync = kt_atomic_pre_op(thr, pc, ad, size, mo, read, write,    \
					sync);                                 \
                                                                               \
		(op);                                                          \
                                                                               \
		sync = kt_atomic_post_op(thr, pc, ad, size, mo, read, write,   \
					 sync);                                \
                                                                               \
		if (sync != NULL)                                              \
...
{
	KT_ATOMIC_OP(kt_atomic8_store_no_ktsan(addr, value), (uptr_t)addr,
		     KT_ACCESS_SIZE_1, mo, false, true);
}

void kt_atomic16_store(kt_thr_t *thr, uptr_t pc, void *addr, u16 value,
		       ktsan_memory_order_t mo)
{
	KT_ATOMIC_OP(kt_atomic16_store_no_ktsan(addr, value), (uptr_t)addr,
		     KT_ACCESS_SIZE_2, mo, false, true);
}

void kt_atomic32_store(kt_thr_t *thr, uptr_t pc, void *addr, u32 value,
		       ktsan_memory_order_t mo)
{
	KT_ATOMIC_OP(kt_atomic32_store_no_ktsan(addr, value), (uptr_t)addr,
		     KT_ACCESS_SIZE_4, mo, false, true);
}

void kt_atomic64_store(kt_thr_t *thr, uptr_t pc, void *addr, u64 value,
		       ktsan_memory_order_t mo)
{
	KT_ATOMIC_OP(kt_atomic64_store_no_ktsan(addr, value), (uptr_t)addr,
		     KT_ACCESS_SIZE_8, mo, false, true);
}
		   ...
		   ktsan_memory_order_t mo)
{
	...
	KT_ATOMIC_OP(rv = kt_atomic8_load_no_ktsan(addr), (uptr_t)addr,
		     KT_ACCESS_SIZE_1, mo, true, false);
		     ...
		     ktsan_memory_order_t mo)
{
	...
	KT_ATOMIC_OP(rv = kt_atomic16_load_no_ktsan(addr), (uptr_t)addr,
		     KT_ACCESS_SIZE_2, mo, true, false);
		     ...
		     ktsan_memory_order_t mo)
{
	...
	KT_ATOMIC_OP(rv = kt_atomic32_load_no_ktsan(addr), (uptr_t)addr,
		     KT_ACCESS_SIZE_4, mo, true, false);
		     ...
		     ktsan_memory_order_t mo)
{
	...
	KT_ATOMIC_OP(rv = kt_atomic64_load_no_ktsan(addr), (uptr_t)addr,
		     KT_ACCESS_SIZE_8, mo, true, false);
		       ...
		       ktsan_memory_order_t mo)
{
	KT_ATOMIC_OP(kt_atomic_set_bit_no_ktsan(addr, nr),
		     (uptr_t)addr + (nr >> 3), KT_ACCESS_SIZE_1, mo, false,
		     true);
}
			  ...
			  ktsan_memory_order_t mo)
{
	KT_ATOMIC_OP(kt_atomic_change_bit_no_ktsan(addr, nr),
		     (uptr_t)addr + (nr >> 3), KT_ACCESS_SIZE_1, mo, true,
		     true);
}
