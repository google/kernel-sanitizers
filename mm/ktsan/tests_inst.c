...
void kt_test(thr_func_t setup, thr_func_t teardown, thr_func_t first,
	     thr_func_t second, const char *name, bool on_stack, bool has_race)
{
...
{
	kt_test(kt_nop, kt_nop, slab_race_read, slab_race_write, "slab-race",
		false, true);
}
...
void kt_uaf_setup(void *p)
{
	...
	arg->cache = kmem_cache_create("uaf_cache", sizeof(struct uaf_obj), 0,
				       0, NULL);
	BUG_ON(!arg->cache);
...
static void sdbr_setup(void *p)
{
	...
	arg->cache = kmem_cache_create("sdbr_cache", sizeof(struct sdbr_obj), 0,
...
{
	kt_test(kt_nop, kt_nop, offset_first, offset_second, "offset", false,
		false);
}
...
{
	kt_test(kt_nop, kt_nop, spinlock_first, spinlock_second, "spinlock",
		false, false);
}
...
{
	kt_test(kt_nop, kt_nop, atomic_first, atomic_second, "atomic", false,
		false);
	kt_test(kt_nop, kt_nop, atomic64_first, atomic64_second, "atomic64",
		false, false);
...
{
	kt_test(kt_nop, kt_nop, compl_first, compl_second, "completion", false,
		false);
}
...
{
	kt_test(kt_nop, kt_nop, mutex_first, mutex_second, "mutex", false,
		false);
}
...
{
	kt_test(kt_nop, kt_nop, sema_first, sema_second, "semaphore", false,
		false);
}
...
{
	kt_test(kt_nop, kt_nop, rwlock_first, rwlock_second, "rwlock", false,
		false);
}
...
{
	kt_test(thr_crt_main, kt_nop, thr_crt_first, kt_nop, "thread creation",
		false, false);
}
...
static void kt_test_percpu(void)
{
	...
	kt_test(kt_nop, kt_nop, percpu_irq, percpu_irq, "percpu irq", false,
		false);
	...
	kt_test(kt_nop, kt_nop, percpu_race, percpu_race, "percpu race", false,
		true);
}
...
static void seq_read1(void *p)
{
	...
	for (i = 0; i < 1000; i++) {
		do {
			...
			rmb();
			sum = arg->data[0] + arg->data[1] + arg->data[2] +
			      arg->data[3];
			rmb();
...
{
	kt_test(seq_main, kt_nop, seq_write1, seq_read1, "seqcount1", false,
		false);
	kt_test(seq_main, kt_nop, seq_write1, seq_read2, "seqcount2", false,
		false);
	kt_test(seq_main, kt_nop, seq_write1, seq_read3, "seqcount3", false,
		false);
	kt_test(seq_main, kt_nop, seq_write4, seq_read4, "seqcount4", false,
		false);
...
static void kt_test_malloc(void)
{
	...
	kt_test(kt_nop, kt_nop, kt_malloc1, kt_malloc2, "kmalloc", false, true);
}
...
{
	kt_test(bu_setup, kt_nop, bu_lock, bu_unlock, "bad-unlock", false,
		true);
}
