			    ...
			    kt_trace_state_t *state)
{
	...
	if (event.type != kt_event_nop && event.type != kt_event_func_enter &&
	    event.type != kt_event_func_exit &&
	    event.type != kt_event_thr_stop &&
	    event.type != kt_event_thr_start && event.type != kt_event_lock &&
	    event.type != kt_event_unlock && event.type != kt_event_rlock &&
